<template>
    ...
    <div class=" bg-slate-200 mx-4 rounded-lg shadow-sm mb-10">
        <table class=" w-full overflow-scroll">
            ...
            <tbody>
                <tr class="odd:bg-white even:bg-slate-50" v-for="assetsType in assetsTypeState.assetsTypes"
                    v-bind:key="assetsType._id">
                    <td>{{ assetsType.name }}</td>
                    <td class=" screen-md:hidden">{{ assetsType.addedBy.name }}</td>
                    <td class=" screen-md:hidden">{{ formatDate(assetsType.createdAt) }}</td>
                    <td class=" screen-md:hidden">{{ assetsType.updatedBy.name }}</td>
                    <td class=" screen-md:hidden">{{ formatDate(assetsType.updatedAt) }}</td>
                    <td @click="openPopup(assetsType._id, assetsType.name)"><i class="fa fa-pen hover:text-green-500"></i>
                    </td>
                    <td @click="deleteAssetsType(assetsType._id)"><i class="fa fa-trash-can hover:text-red-500"></i></td>
                </tr>
            </tbody>
        </table>
    </div>
    ...
    <div id="popup-modal" v-if="popup"
        ...
        <div class="relative bg-white rounded-lg shadow self-center px-20  max-w-md m-auto">
            ...
            <form class=" grid py-20" @submit.prevent="update" method="POST">
                <div class="">
                    <input id="name" name="name" type="name" v-model="updateAssetsType.name" autocomplete="name"
                        required=""
...
export default {
    ...
    methods: {
        submit: async function () {
            this.$store.dispatch("createAssetsType", { name: this.assetsType.name, addedBy: this.userState.user._id, updatedBy: this.userState.user._id });
        },
        update: async function () {
            this.$store.dispatch("updateAssetsType", { _id: this.updateAssetsType._id, name: this.updateAssetsType.name, updatedBy: this.userState.user._id });
            this.closePopup();
        },
        formatDate(value) {
            if (value) {
        ...
        deleteAssetsType(id) {
            this.$store.dispatch("deleteAssetsType", id);
        },
        openPopup(id, name) {
            this.updateAssetsType.name = name;
            this.updateAssetsType._id = id;
